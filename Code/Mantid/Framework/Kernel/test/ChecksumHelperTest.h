...
public:
  ...
  void testSha1FromString() {
    std::string response =
        ChecksumHelper::sha1FromString("Test this string out for size");
    TSM_ASSERT_EQUALS("The calculated SHA-1 hash is not as expected",
                      "c1c9af231c340826bdabd33eae076d5e532eba08", response);
  }
  ...
  void testSha1FromFile() {
    ...
    TSM_ASSERT_EQUALS("The calculated SHA-1 hash is not as expected",
                      "363cbe9c113b8bcba9e0aa94dbe45e67856ff26b", response);
  ...
  void testGitSha1FromFile() {
    ...
    TSM_ASSERT_EQUALS("The calculated git-hash is not as expected",
                      "db46957d5afdb266b4b3321f3ce2b8887f190ff5", response);
    Poco::File(filename).remove();
  }

  void testGitSha1FromFileWithLinuxLineEndings() {
    const std::string filename(
        "ChecksumHelperTest_testGitSha1FromFileWithLinuxLineEndings.txt");
    ...
    std::string response = ChecksumHelper::gitSha1FromFile(filename);
    TSM_ASSERT_EQUALS("The calculated git-hash is not as expected",
                      "7e78655a4e48aa2fbd4a3f1aec4043009e342e31", response);
    Poco::File(filename).remove();
  }
