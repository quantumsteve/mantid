...
void VesuvioResolution::cacheResolutionComponents(
  ...
  if ((m_mass - 1.0) > DBL_EPSILON) {
    ...
    gsl_poly_solve_quadratic(m_mass - 1.0, 2.0 * std::cos(theta),
                             -(m_mass + 1.0), &x0, &x1);
  ...
  if (m_mass > 1.0) {
    ...
    double r2 = 1.0 - l2l1 * k0k1p3 +
                l2l1 * std::pow(k0k1, 2) * std::cos(theta) -
                k0k1 * std::cos(theta);
    ...
    double factor = (0.2413 * 2.0 / k1) *
                    std::abs((std::cos(theta) + l2l1) / std::sin(theta));
    m_lorentzFWHM = hwhmLorentzE * factor;
    wgauss = hwhmGaussE * factor;
  }

  double k0y0 = k1 * k0k1; // k0_y0 =  k0 value at y=0
  double wtheta = 2.0 * STDDEV_TO_HWHM *
                  std::abs(k0y0 * k1 * std::sin(theta) / qy0) * respar.dthe;
  double common = (m_mass / mn) - 1 + k1 * std::cos(theta) / k0y0;
  double wl1 = 2.0 * STDDEV_TO_HWHM *
               std::abs((std::pow(k0y0, 2) / (qy0 * detpar.l1)) * common) *
               respar.dl1;
  double wl2 = 2.0 * STDDEV_TO_HWHM *
               std::abs((std::pow(k0y0, 3) / (k1 * qy0 * detpar.l1)) * common) *
               respar.dl2;
 ...
 */
void VesuvioResolution::setAttribute(const std::string &name,
 ...
 * The area is normalized to lorentzAmp
 * @param voigt [Out] Output values (vector is expected to be of the correct
 * size
 * @param xValues Input coordinates
 * @param lorentzPos LorentzPos parameter
