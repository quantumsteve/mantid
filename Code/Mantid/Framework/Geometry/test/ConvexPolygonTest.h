...
public:
  ...
  void
  test_Buildling_With_Head_Vertex_With_Less_Than_Two_Other_Points_Throws() {
    Vertex2D *origin = new Vertex2D;
  ...
  void test_Extreme_Points_Are_Correct() {
    ...
    TS_ASSERT_DELTA(parallelogram.largestX(), 2.0 + 0.5 * std::sqrt(2.0),
                    DBL_EPSILON);
    TS_ASSERT_DELTA(parallelogram.smallestY(), 0.0, DBL_EPSILON);
    TS_ASSERT_DELTA(parallelogram.largestY(), 0.5 * std::sqrt(2.0),
                    DBL_EPSILON);
  }
...
private:
  ...
  ConvexPolygon makeRectangle() { return ConvexPolygon(0.0, 2.0, 0.0, 1.0); }
  ...
  ConvexPolygon makeParallelogram() {
    ...
    head->insert(new Vertex2D(2.0, 0.0));
    head->insert(
        new Vertex2D(2.0 + 0.5 * std::sqrt(2.0), 0.5 * std::sqrt(2.0)));
  ...
  // TS_ASSERT_THROWS()
  template <typename ExceptionType> void doExceptionCheck(Vertex2D *vertex) {
    try {
