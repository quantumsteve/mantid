...
public:
  ...
  void testRelativeRotate() {
    ...
    // but the absolute pos should have changed due to the parent's rotation
  ...
  void testRotation() {
    // Very simple test here, not sure my brain can handle anything more
    // complicated!
    ...
    // Check we get back just the child's rotation if we set the parent's to
    // none
