...
private:
  ...
  */
  IDFEnvironment create_idf_and_vtp_pair(bool put_vtp_next_to_IDF = true) {
    ...
    ScopedFile vtp(vtp_file_contents, vtp_filename,
                   instrument_dir); // Put the vtp file adjacent to the idf.

    if (!put_vtp_next_to_IDF) {
      vtp =
          ScopedFile(vtp_file_contents,
                     vtp_filename); // Overwrite to put vtp file in the temp dir
    }
...
public:
  ...
  void test_extract_ref_info() {
    ...
    TS_ASSERT_THROWS_NOTHING(
        parser.initialize(filename, "For Unit Testing", xmlText););
                                          ...
                                          // Definition File
  {
    ...
    InstrumentDefinitionParser parser;
    TS_ASSERT_THROWS_NOTHING(
        parser.initialize(filename, "For Unit Testing2", xmlText););
    TS_ASSERT_THROWS_NOTHING(i = parser.parseXML(NULL););
  ...
  void test_parse_RectangularDetector() {
    ...
    InstrumentDefinitionParser parser;
    TS_ASSERT_THROWS_NOTHING(
        parser.initialize(filename, "RectangularUnitTest", xmlText););
    TS_ASSERT_THROWS_NOTHING(i = parser.parseXML(NULL););
  ...
  void testAdjust() {
    ...
    InstrumentDefinitionParser parser;
    TS_ASSERT_THROWS_NOTHING(
        parser.initialize(filename, "AdjustTest", xmlText););
    TS_ASSERT_THROWS_NOTHING(i = parser.parseXML(NULL););
  ...
  void testUseAdjacentCacheFile() {
    ...
    EXPECT_CALL(*mockCache, getLastModified())
        .WillOnce(Return(largerTime)); // Mock expectation set such that Cache
                                       // file modified created most recently,
                                       // so SHOULD be used.
    ...
    parser.initialize(idf, cache, instrumentEnv._instName,
                      instrumentEnv._xmlText);
    TS_ASSERT_THROWS_NOTHING(parser.parseXML(NULL));

    TS_ASSERT_EQUALS(InstrumentDefinitionParser::ReadAdjacent,
  ...
  void testWriteAdjacentCacheFileIfCacheIsOutOfDate() {
    ...
    EXPECT_CALL(*mockIDF, getLastModified())
        .WillRepeatedly(Return(largerTime)); // Mock expectation set up so that
                                             // IDF will appear newer than cache
                                             // file.
    EXPECT_CALL(*mockCache, getLastModified())
    ...
    TS_ASSERT_EQUALS(InstrumentDefinitionParser::WroteCacheAdjacent,
                     parser.getAppliedCachingOption()); // Check that the
                                                        // adjacent cache file
                                                        // was used.
    TS_ASSERT(Mock::VerifyAndClearExpectations(mockIDF));
  ...
  void testReadFromCacheInTempDirectory() {
    ...
    EXPECT_CALL(*mockCache, exists())
        .WillRepeatedly(Return(false)); // Mock expectation set such that
                                        // adjacent Cache file does not exist,
  ...
  */
  void testWriteCacheFileToTempDirectoryIfNoIDF() {
    ...
    EXPECT_CALL(*mockCache, exists())
        .WillRepeatedly(Return(true)); // Mock expectation set such that Cache
                                       // file does exist, but should not be
                                       // used..
  ...
  */
  void testIDFFileWithMissingDetectorIDList() {
    ...
    InstrumentDefinitionParser parser;
    TS_ASSERT_THROWS_NOTHING(
        parser.initialize(idfFilename, "For Unit Testing", idfFileContents););
