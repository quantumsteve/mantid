...
namespace {
/**
 * Proxy to return an API::IEventWorkspace_sptr rather than
...
BOOST_PYTHON_MODULE(WorkspaceCreationHelper) {
  ...
  typedef MatrixWorkspace_sptr (*Signature1_2D)(
      int nHist, int nBins, bool includeMonitors, bool startYNegative);
  // Forces it to return a MatrixWorkspace pointer rather than Workspace2D
  typedef MatrixWorkspace_sptr (*Signature2_2D)(int numBanks, int numPixels,
                                                int numBins);
  ...
  typedef IMDHistoWorkspace_sptr (*Signature1_MDHisto)(
